...
def run_mace4(fm="", output_dir="temp-output", out_name="temp-output", axiom="../axioms/everything", domain_size=2, max_models=1, max_seconds=60, model_output=0):
    os.system("cat " + axiom + " > temp-input")
    ...
    os.system(f"../mace4.src/mace4 -n {domain_size} -s 60 -b -1 -N -1 -i 1 -m {max_models} -t {max_seconds} -P {model_output} -p 0 -f temp-input > ../outputs/{output_dir}/{out_name}.out")
